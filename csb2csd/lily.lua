...
local function write_csd(rb_csdname, xml_doc)
  ...
  -- local l,n = rb_csdname:find('%/')
  -- local l, n = rb_csdname:gsub("%/$", "/")
  -- local n =rb_csdname:split('\\/')
  ...
  if diff_result and rc == 1 then
    ...
    if csd_doc then
      ...
      print(xml.tostring(diff_doc, "", "  "):sub(2, -1))
...
local function process(files)
  ...
  -- if not c then
  --   filename= files[1] .. '/' ..files[2]
  -- else
  --   filename= files[1] .. '/' ..files[2] .. '/' ..files[3]
  ...
  if rc == 1 then
    ...
    -- print("name:", name)
    -- if c then
    --   local str = "mkdir -p " .. files[1] .. "/" .. files[2]
    --   local str = "mkdir -p " .. csd_dir
    --   os.execute(str)
